...
int codec_2pass_get_quant(CODEC* codec, xvid_enc_frame_t* frame)
{
...
//	DWORD read;
	...
	if (twopass->nns_array_pos >= twopass->nns_array_length)
	{
			...
			codec->framenum < twopass->nns_array_length + codec->config.max_bframes)
		{
			...
			frame->quant = 2;	/* XXX: ugly eof fix */
			return ICERR_OK;
		}
		else
		{
	...
	if (credits_pos)
	{
		if (codec->config.mode == DLG_MODE_2PASS_2_INT)
		{
			...
			case CREDITS_MODE_QUANT :
				...
				{
					frame->quant = (frame->type==XVID_TYPE_IVOP) ?
	...
	if (frame->type == XVID_TYPE_IVOP && !credits_pos)
	{
	...
	else if (frame->type != XVID_TYPE_IVOP)
	{
...
int codec_2pass_update(CODEC* codec, xvid_enc_stats_t* stats)
{
	...
	if (codec->twopass.nns_update_pos == 0)
	{
		total_size = 0;
	}
	...
	{
	case DLG_MODE_2PASS_1 :
		nns1.bytes = stats->length;	// total bytes
		nns1.dd_v = 0;
...
//		nns1.quant = stats->quant;
		...
		total_size += stats->length;

		DEBUG1ST(stats->length, (int)total_size/1024, frame_type, stats->quant, quant_type, stats->kblks, stats->mblks)
	...
	case DLG_MODE_2PASS_2_INT :
	case DLG_MODE_2PASS_2_EXT :
		credits_pos = codec_is_in_credits(&codec->config, codec->twopass.nns_update_pos);
			if (!credits_pos)
			{
				codec->twopass.quant_count[stats->quant]++;
				//if ((codec->twopass.nns1.quant & NNSTATS_KEYFRAME))
                if ((codec->twopass.nns1_array[codec->twopass.nns_update_pos].quant & NNSTATS_KEYFRAME))
				{
                    ...
                    // XXX: very ugly hack for bframs; this affects twopass quality
                    codec->twopass.KFoverflow = codec->twopass.nns2_array[codec->twopass.nns_update_pos].bytes/*codec->twopass.desired_bytes2*/ - stats->length;
				...
				codec->twopass.overflow += codec->twopass.nns2_array[codec->twopass.nns_update_pos].bytes/*codec->twopass.desired_bytes2*/ - stats->length +
					codec->twopass.KFoverflow_partial;
        ...
        */
		...
		wsprintf(tmp, "[%i] 2nd-pass: quant:%d %s %s stats1:%d scaled:%d actual:%d overflow:%d %s", 
			codec->twopass.nns_update_pos,
			stats->quant,
			quant_type,
			frame_type,
			codec->twopass.nns1_array[codec->twopass.nns_update_pos].bytes,
			codec->twopass.nns2_array[codec->twopass.nns_update_pos].bytes, /* codec->twopass.desired_bytes2, */
			stats->length,
			codec->twopass.overflow,
			credits_pos ? "credits" : "movie");
